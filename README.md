...
- **多维度筛选**：支持16个维度字段的灵活筛选（起保年度、周序号、客户类别、机构层级、三级机构/城市、业务类型分类、险种类型、是否新能源车、险别组合、是否过户车、新续转状态、车险评级、高速风险等级、大货车评...
- **对比分析**：同比、环比及自定义时间段对比
- **响应式设计**：适配桌面、平板和移动设备
- **实时计算**：基于标准公式的动态指标计算
- **智能图表洞察**：客户类别旭日图支持内/外环绝对值指标切换，条形图自动缩放极值，所有图表标题会随数据自动生成核心观点
