...
npx tsc --noEmit           # 手动运行 TypeScript 类型检查（脚本未内置）
```
...
├── components/
│   ├── dashboard/          # 核心仪表板业务组件
│   │   ├── header.tsx      # 顶部导航，包含时间周期切换入口
│   │   ├── time-period-toggle.tsx
│   │   ├── kpi-grid.tsx
│   │   ├── weekly-trend.tsx
│   │   ├── customer-performance.tsx
│   │   ├── comparison-analysis.tsx
│   │   └── ai-deterioration-analysis.tsx
│   └── ui/                 # shadcn/ui 基础UI组件
├── contexts/
...
│   └── use-kpi-data.ts
├── lib/
│   ├── kpi-calculator.ts   # 年累计 / 周度 KPI 计算逻辑
│   ├── utils.ts            # 工具函数（含增量计算、图表聚合）
│   ├── data.ts             # 默认数据与配置
│   └── types.ts            # 全局类型定义
└── ……（其余辅助模块）
```
...
### 时间周期切换
- `TimePeriodToggle` 组件负责在顶部导航中切换“年累计 / 当周”模式。
- `data-context.tsx` 提供 `timePeriod` 状态，并通过 `use-filtered-data`、`use-kpi-data` 组合输出两套数据（累计值与周度增量）。
- `lib/kpi-calculator.ts`、`computeDeltaRows` 与 `aggregateChartData` 负责在 KPI 和图表层面完成累计/增量转换。
