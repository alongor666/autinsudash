...
**API密钥管理**：
1. **环境变量**: 使用 `.env.example` 作为模板，创建本地 `.env` 文件
2. **密钥配置**: 从 Google AI Studio (https://aistudio.google.com/app/apikey) 获取 Gemini API 密钥
3. **生产部署**: 通过环境变量注入，避免在代码中硬编码密钥
4. **日志安全**: 生产环境已禁用调试日志，仅保留关键错误日志

**构建安全**：
- 已启用 TypeScript 和 ESLint 错误检查，确保代码质量
- 所有构建必须通过类型检查和代码规范检查
